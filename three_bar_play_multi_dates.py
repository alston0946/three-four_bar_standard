...
# =========================
# GitHub 仓库路径 / 环境变量配置
# 说明：以下仅用于 GitHub Actions 运行适配，不改动后面的交易筛选逻辑。
# =========================
...
def _env_optional_int(name: str):
    ...
    if not raw or raw.lower() in {"none", "null"}:
        return None
    value = int(raw)
    # TEST_LIMIT=0 视为不限制，避免 GitHub Secrets 空/0 导致扫描 0 只股票
...
def _parse_target_dates(raw: str):
    dates = []
...
def get_target_dates():
    # 优先支持多日期 TARGET_DATES；其次兼容单日期 TARGET_DATE；都没有则用中国当天日期。
    raw_multi = _env_text("TARGET_DATES")
    raw_single = _env_text("TARGET_DATE")

    if raw_multi:
        dates = _parse_target_dates(raw_multi)
    elif raw_single:
        dates = _parse_target_dates(raw_single)
    else:
        dates = [get_cn_today()]

    if not dates:
        raise ValueError("TARGET_DATE / TARGET_DATES 日期格式错误，必须是 YYYYMMDD，例如 20260515")
    return dates
...
# Tushare Token
# =========================
TUSHARE_TOKEN = _env_text("TUSHARE_TOKEN")
...
START_DATE = _env_text("START_DATE", "20240301")


# =========================
# 输出文件
# =========================
OUTPUT_FILE = os.path.join(OUTPUT_DIR, f"three_bar_play_resting_candidates_{END_DATE}.csv")
DEBUG_FILE = os.path.join(OUTPUT_DIR, f"three_bar_play_resting_debug_rejected_{END_DATE}.csv")
FAILED_FILE = os.path.join(OUTPUT_DIR, f"three_bar_play_resting_failed_fetch_{END_DATE}.csv")
FILTERED_FILE = os.path.join(OUTPUT_DIR, f"three_bar_play_resting_filtered_out_{END_DATE}.csv")
...
TEST_LIMIT = _env_optional_int("TEST_LIMIT")
BATCH_START = _env_int("BATCH_START", 0)
BATCH_SIZE = _env_int("BATCH_SIZE", 10000)
SLEEP_SEC = _env_float("SLEEP_SEC", 0.10)
...
# 单股多日期评估
# =========================
def evaluate_one_stock_multi_dates(pro, ts_code: str, ticker: str, name: str, target_dates):
    matched_list = []
    debug_list = []
    failed_list = []
...
def main():
    ...
    print("\n2) 开始 3 Bar Play 日线候补池扫描（resting bar 版）...")
    ...
    print("\n候选结果已保存:", OUTPUT_FILE)
    print("调试未通过已保存:", DEBUG_FILE)
    print("抓取失败已保存:", FAILED_FILE)
    print("已过滤股票已保存:", FILTERED_FILE)
