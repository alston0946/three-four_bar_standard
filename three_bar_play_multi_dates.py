...
def _env_int(name: str, default: int, none_if_nonpositive: bool = False):
    raw = (os.getenv(name) or "").strip()
    if raw == "" or raw.lower() in {"none", "null"}:
        return None if none_if_nonpositive else default
    value = int(raw)
    if none_if_nonpositive and value <= 0:
        return None
    return value
...
TEST_LIMIT = _env_int("TEST_LIMIT", 0, none_if_nonpositive=True)
