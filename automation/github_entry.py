...
def main() -> int:
    target_date = os.environ.get("TARGET_DATE", sg_today_str())
    ...
    try:
        ...
        body = "\n".join(body_parts)
        send_email(subject, body, attachments=paths.values())
        print(body)
        return 0
    except Exception:
        ...
        print(tb)
        try:
            send_email(subject + " FAILED", tb)
        except Exception:
            print("发送失败邮件也失败了", file=sys.stderr)
        return 1
